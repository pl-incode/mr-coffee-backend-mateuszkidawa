...
app.route("/users/new").get((req, res, next) => {
  res.render("newUser", {});
});
...
app.route("/users/:id").get((req, res, next) => {
  ...
  idNumber >= myData.users.length
    ? res.render("user", {
        Title: "No such user",
      })
    : res.render("user", {
        Title: `User ${idNumber}`,
        user: myData.users[idNumber],
      });
});

app.route("/users/:id/schedules").get((req, res, next) => {
  const idNumber = req.params.id;
  ...
  if (arr.length < 1) {
    res.render("schedule", {
      Title: "Make an appointemnt",
    });
    return;
  }
  res.render("schedule", {
    Title: `User ${idNumber} schedule`,
    schedule: arr,
  });
});

app.route("/schedules/new").get((req, res, next) => {
  res.render("newSchedule", {});
});

app.listen(3003, () => {
  console.log(`http://localhost:3003/ is waiting for requests.`);
});
